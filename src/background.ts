(() => {
  ...
  const MAJORITY_VALUES: {
    ...
    spoofed: Required<BrowserValues>;
  } = {
    headers: {
      "user-agent":
  ...
  // オリジナルのブラウザ値を保存
  const ORIGINAL_VALUES: BrowserValues = {
  ...
  function getSettings(): Promise<Settings> {
    return new Promise((resolve) => {
      browserAPI.storage.local.get("settings", (result: { settings?: Settings }) => {
        // デフォルト設定
  ...
  browserAPI.webRequest.onBeforeSendHeaders.addListener(
    async (details) => {
      ...
      const hasIfNoneMatchHeader = headers.some(
        (header) => header.name.toLowerCase() === "if-none-match"
      );
      ...
      const hasUpgradeHeader = headers.some(
        (header) => header.name.toLowerCase() === "upgrade-insecure-requests"
      );
  ...
  // すべてのタブに設定変更を通知する関数
  function notifyAllTabs(settings: Settings): void {
    // 現在開いているすべてのタブを取得
    browserAPI.tabs.query({}, (tabs) => {
      for (const tab of tabs) {
        ...
        try {
          ...
          console.error(
            `[Ununique Debug] タブID: ${tab.id} への設定変更通知中にエラーが発生しました: ${e.message}`
          );
        }
      }
    });
